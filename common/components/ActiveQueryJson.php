...
class ActiveQueryJson extends ActiveQuery
{
     ...
     */
    public function jsonExtractWhere($column, $value, $operate = '=', string $tableName = '')
    {
        list($cond, $jsonCond) = $this->splitJsonColumn($column, true);
        $tableName = isset($tableName) && !empty(trim($tableName)) ? $tableName . '.' : '';
        $condition = "JSON_EXTRACT({$tableName}$cond, '$jsonCond') $operate $value";
